...
public class MarkovChain
{
    ...
    public void lengthChain(ArrayList<Note> song)
    {
        ...
        possibleDur = new Long[song.size()];
        for(int i = 0; i < song.size(); i += 1)
        {
            possibleDur[i]=(song.get(i).getDuration());
            letters[i] = song.get(i).getName();
        }
        NoRepeats possibleDurNoRepeats = new NoRepeats();
        possibleDurSet = possibleDurNoRepeats.removeDuplicates(possibleDur);
        this.probibsDur = new double[keys][possibleDurSet.length];
        HashMap<Long, Integer> durToIndexPos = new HashMap<>();
        //Set the occurences of each note to 0
        //init the hashmaps
        ...
        //init durToindexpos
        for(int i = 0; i < possibleDurSet.length; i += 1)
        {
            durToIndexPos.put(possibleDurSet[i], i);
        }

        for(int row = 0; row < keys; row += 1)
        {
            for(int col = 0; col < possibleDurSet.length; col += 1)
            {
        ...
        for(Note notes : song)
        {
            ...
            occurences.put(notes.getName(), occurences.get(notes.getName()) + 1);
            if(notes.getName() == letters[letters.length-1])
            {
                occurences.put(notes.getName(), occurences.get(notes.getName()) - 1);
            }
        }
    ...
    public Note getNext(Note Start)
    {
        ...
        //     if (total >= target)
        //     {
        //         finalNote.setDuration(possibleDurSet[i]);
        //     }
        //     i += 1;
        // }
        return finalNote;
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        ArrayList<Note> originalSong = myReader.getNewSong("Birthday.mid");
        ...
        for(int i = 0; i < mySong.size(); i +=1)
        {
            ...
            System.out.print(": " + mySong.peek().getDuration());
